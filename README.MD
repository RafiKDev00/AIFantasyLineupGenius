...
How did we do? We came first in the regular season, and finished with 11 wins and 3 losses, Losing in the Semi Finals to end up taking 3rd place in the leauge. We Scored the second most points in the leauge as well 1750, with the first place (and most scoring) team scoring 1800, and the next most scoring (the fifth place team) at 1680. In the end a rash of injuries doomed the team, although the bot failed to pick up on them (as expected) and did not manage the lineup effectivcely afterward (this forced me to step in).

Nonetheless a sucesfull trial run. Next season: live draft instead of manual querying, injury detection and bench managed, upgrade the model and model querying to track more live data (maybe allow a pre game twitter scrape?). Let's see....
